...
namespace QuanLyQuanBilliards.Forms
{
    public partial class fThanhToan : Form
    {
        ...
        "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    // 1. Cập nhật thông tin giảm giá (giữ nguyên code cũ)
                    ...
                    // 2. Gọi SP Thanh Toán (CẬP NHẬT THÊM THAM SỐ)
                    decimal finalMoney = decimal.Parse(lblFinalTotal.Text.Replace("đ", "").Replace(",", "").Replace(".", ""));

                    // _rawTongTien: Biến toàn cục chúng ta đã khai báo ở đầu class fThanhToan, chứa tổng chưa giảm
                    db.Database.ExecuteSqlCommand(
                        ...
                        new System.Data.SqlClient.SqlParameter("@TongTienTruocGiam", _rawTongTien) // <--- THÊM DÒNG NÀY
                    );
        ...
        // Helper Class nếu chưa có ở namespace ngoài
